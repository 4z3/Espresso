...
//
fs = require('fs');
exports.collectFiles = function (callback, dirname) {

  // TODO use proper search path
  if (!dirname) {
    dirname = this.config.applicationDirectory
      + '/frameworks/The-M-Project/modules/' + this.self.name;
  };

  var framework = this.self.name;
    ...
    if (filenames && filenames.length > 0) {
      ...
      filenames.forEach(function (filename) {
        fs.readFile(filename, function (err, content) {
          if (err) {
            console.error(err.stack); // TODO communicate error
          } else {
            // TODO move this hack to the app module
            var _framework = /\/i18n\//.test(filename)
                ? framework + '/i18n'
                : framework
                ;
            console.log('file:', _framework, '::', filename);
            files[filename] = {
              filename: filename,
