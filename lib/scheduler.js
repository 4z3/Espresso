...
Scheduler.prototype.add = function (name, deps, duty, undercover) {
  var job = new Job(name, deps, duty, undercover);
...
Scheduler.prototype.run = function (callback, results) {
  var that = this;

  if (callback) {
    this.add(
        JSON.stringify(Object.keys(this.jobs)),
        Object.keys(this.jobs),
        callback,
        true);
  };

  // mark or add completed jobs
  var that = this;
  function complete(name) {
    if (!(name in that.jobs)) {
      that.add(name, [], function () {
        throw new Error('This must never be called');
      });
    };
    var job = that.jobs[name];
    if (job.state !== 'completed') {
      that.results[job.name] = job.result = results[name];
      job.restate('completed');
      job.deps.forEach(complete);
    };
  };
  if (results instanceof Object) {
    Object.keys(results).forEach(complete);
  };
      ...
      .forEach(function (job) {
        ...
        job.duty.call(context, function (result) {
          ...
          if (arguments.length > 0) {
            that.results[job.name] = job.result = result;
          };
          reschedule(turn);
        });
      });
